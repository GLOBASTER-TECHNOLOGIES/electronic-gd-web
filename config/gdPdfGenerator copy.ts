...
const loadLocalFont = async (doc: jsPDF) => {
  try {
    ...
    if (!response.ok) {
      throw new Error("Font file missing. Check public/fonts/NotoSansDevanagari-Regular.ttf");
    }
...
export const generateGDPDF = async (gd: GDData) => {
  ...
  const SCALE = 1.1; // 🔥 Global font scaling (10% increase)
  ...
  doc.text("रेलवे सुरक्षा बल / RAILWAY PROTECTION FORCE", 297, 56, { align: "center" });

  doc.setFontSize(10 * SCALE);
  doc.text("दैनिक डायरी (रोजनामचा) / Daily Diary (Roznamacha)", 297, 72, { align: "center" });
  ...
  const dateStr = new Date(gd.diaryDate).toLocaleDateString("en-GB").replace(/\//g, " . ");
  doc.setFontSize(12 * SCALE);
  ...
  doc.setFontSize(10 * SCALE);
  doc.text(gd.post.toUpperCase(), 429, startY - 2); 
  ...
  const tableRows = gd.entries.map((entry) => {
    ...
    const force = entry.signature.forceNumber ? `${entry.signature.forceNumber}` : "";
