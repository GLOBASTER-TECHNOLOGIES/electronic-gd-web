...
import Officer from "@/models/user.model"; // Assuming your Officer/User model is here
  ...
  try {
    ...
    if (!postName || !postCode || !division) {
      return NextResponse.json(
        { success: false, message: "Post Name, Code, and Division are required." },
        { status: 400 }
      );
    }
    ...
    if (existingPost) {
      return NextResponse.json(
        { success: false, message: "A Post with this Name or Code already exists." },
        { status: 409 }
      );
    }
